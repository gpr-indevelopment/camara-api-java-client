...
public class CamaraClient {
    ...
    protected <T> RespostaCamara<T> executarChamada(HttpRequest requisicao, Type tipoResposta) throws IOException, InterruptedException {
    ...
    protected <T> RespostaCamara<T> consultarPorId(String id, String urlBase, Type tipoEsperado) throws IOException, InterruptedException {
        return executarChamada(new RequisicaoBuilder(urlBase).segmentosPath(id).build(), tipoEsperado);
    }

    protected <T> List<T> consultarSemPaginacao(Consulta consulta, String urlBase, Type tipoEsperado, String... segmentosPath) throws IOException, InterruptedException {
        RespostaCamara<List<T>> resposta = executarConsulta(consulta, urlBase, tipoEsperado, segmentosPath);
        return resposta.getDados();
    }

    protected <T> Pagina<T> consultarComPaginacao(Consulta consulta, String urlBase, Type tipoEsperado, String... segmentosPath) throws IOException, InterruptedException {
        RespostaCamara<List<T>> resposta = executarConsulta(consulta, urlBase, tipoEsperado, segmentosPath);
    ...
    private <T> RespostaCamara<List<T>> executarConsulta(Consulta consulta, String urlBase, Type tipoEsperado, String... segmentosPath) throws IOException, InterruptedException {
